...
class TextViewView: UIView {
    ...
    // MARK: - Properties
    let viewModel = BehaviorRelay<TextViewViewViewModel?>(value: nil)
    ...
    private func setup(with viewModel: TextViewViewViewModel) {
        titleLabel.text = viewModel.title
        (textView.rx.text <-> viewModel.text).disposed(by: disposeBag)
    }
